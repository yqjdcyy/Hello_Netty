package com.yao.sample.im.processor;

import com.alibaba.fastjson.JSONObject;
import com.yao.sample.im.protocol.IMDecoder;
import com.yao.sample.im.protocol.IMEncoder;
import com.yao.sample.im.protocol.IMMessage;
import com.yao.sample.im.protocol.IMP;
...
public class MsgProcessor {
    ...
    public void sendMsg(Channel client, String msg) {
        ...
        if (request.getCmd().equals(IMP.LOGIN.getName())) {
            ...
            for (Channel channel : onlineUsers) {
                request = new IMMessage(IMP.SYSTEM.getName(), sysTime(), onlineUsers.size(), channel != client ? getNickName(client) + "加入" : "已与服务器建立连接！");
            ...
            for (Channel channel : onlineUsers) {
                ...
                if ("Console".equals(channel.attr(FROM).get()) & !bSelf) {
                    channel.writeAndFlush(request);
                    continue;
                }
                String content = encoder.encode(request);
